...
def sample_prob_dist(prob_dist):
    ...
    # After looping through all value of 'prob_dist', if
...
def prob_dist_to_one_hot(prob_dist):
    ...
    one_hot = np.zeros(shape = [1, (len(string.ascii_lowercase) + 1)],
                       dtype = np.float)
...
# is generated by the RNN. This lets me see how the RNN is doing in terms of
...
def random_distribution():
    ...
    random_dist = np.random.uniform(0.0, 1.0,
                                    size = [1, (len(string.ascii_lowercase) + 1)])
