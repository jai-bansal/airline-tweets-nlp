...
I will conduct sentiment analysis in 3 ways.  
* Lexicon based (with pre-provided lists of positive and negative terms)   
* Classification Model

**Lexicon Based**:

```{r sentiment_analysis_lexicon_based_automatic, include = T, echo = F}
  ...
  # Translate polarity into a 'predicted_sentiment'.
  ...
  tweet_data$predicted_sentiment = ifelse(tweet_data$polarity < 0, 'negative', 
...
Using these definitions, `r nrow(tweet_data[airline_sentiment == predicted_sentiment])` out of `r nrow(tweet_data)` (`r round(100 * nrow(tweet_data[airline_sentiment == predicted_sentiment]) / nrow(tweet_data), 2)`)% of tweets are predicted correctly.

Broken down by category:   
`r nrow(tweet_data[airline_sentiment == predicted_sentiment & airline_sentiment == 'positive'])` out of `r nrow(tweet_data[airline_sentiment == 'positive'])` (`r round(100 * nrow(tweet_data[airline_sentiment == predicted_sentiment & airline_sentiment == 'positive']) / nrow(tweet_data[airline_sentiment == 'positive']), 2)`)% of positive tweets are predicted correctly.

`r nrow(tweet_data[airline_sentiment == predicted_sentiment & airline_sentiment == 'neutral'])` out of `r nrow(tweet_data[airline_sentiment == 'neutral'])` (`r round(100 * nrow(tweet_data[airline_sentiment == predicted_sentiment & airline_sentiment == 'neutral']) / nrow(tweet_data[airline_sentiment == 'neutral']), 2)`)% of neutral tweets are predicted correctly.

`r nrow(tweet_data[airline_sentiment == predicted_sentiment & airline_sentiment == 'negative'])` out of `r nrow(tweet_data[airline_sentiment == 'negative'])` (`r round(100 * nrow(tweet_data[airline_sentiment == predicted_sentiment & airline_sentiment == 'negative']) / nrow(tweet_data[airline_sentiment == 'negative']), 2)`)% of negative tweets are predicted correctly.

**Classification Model**:

```{r sentiment_analysis_classification_model, include = T, echo = F}
```