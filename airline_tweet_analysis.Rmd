...
```{r sentiment_analysis_naive_bayes, include = T, echo = F}
  ...
  tweet_tdm_train$pred = training_pred
  tweet_tdm_test$pred = test_pred
...
The training error is `r round(nrow(tweet_tdm_train[airline_sentiment != pred]) / nrow(tweet_tdm_train), 2)`.
`r t`
The test error is `r round(nrow(tweet_tdm_test[airline_sentiment != pred]) / nrow(tweet_tdm_test), 2)`.
